...
function getTSParams(streams,from_date,to_date) {
    const body = {
        dataType: "raw",
        streams: streams,
        dataFormat: "dygraphs",
...
function getLatestParams(stream) {
    const body = {
        dataType: "latest",
        streams: stream,
        dataFormat: "live",
...
function requestTSData(conn, id, streams,from_date,to_date) {
    const payload = {
...
function requestLatestData(conn, id, streams) {
    const payload = {
        method: ApiMethods.GetUserData,
        params: {
...
export default class DataManager {
    constructor(conn, tsdataUpdater, livedataUpdater, errorHandler) {
        this.conn = conn;
        this.conn.onmessage = function (event) {
            console.log(event)
            const data = JSON.parse(event.data)
            console.log("Status=" + data.status)
            if (data.status == "success" && Object.hasOwn(data.result,'data') ) { // time-series response has data property
                tsdataUpdater(data.result.data)
            } else if (data.status == "success" && Object.hasOwn(data.result[0],'value')) { // a single 'latest' reposnse has value property
    ...
    */
    RequestTSData(id,streams,from_date,to_date) {
