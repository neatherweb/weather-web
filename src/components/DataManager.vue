...
function ErrorHandler(event) {
    ...
    toast.add({ severity: 'error', summary: 'Disconnected', detail: 'Lost connection to weather data service. Reload the page to try again.', life: 4000 });
}
...
const conn = new WebSocket("wss://api-onset-prod.scriptrapps.io//RThGMDEzNDc3NA==")
const dm = new DataManager(conn,TSDataUpdater,LiveDataUpdater,ErrorHandler)
conn.onopen = function (event) {
    console.log(event)
    console.log("Successfully connected to the websocket server...")
    toast.add({ severity: 'success', summary: 'Connected', detail: 'Connected to weather data service', life: 2000 });
    const currentTime = Date.now()
    dm.RequestTSData("ts-all",[dm.Streams.WindSpeed,dm.Streams.GustSpeed,dm.Streams.WindDirection],currentTime-(1000*3600),currentTime)
